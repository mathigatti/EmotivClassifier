...
def plot_confusion_matrix(cm, classes,
    ...
    plt.tight_layout()
    plt.savefig('education_level_' + title.lower().replace(" ","_") + ".png")
...
target_json = json.load(open(rootPath + 'nivel_educativo_tutor.json'))
...
for band in ['beta','theta','alpha']:
  ...
  for file in os.listdir(rootPath):
    if file[-4:] == '.csv' and file[:5].upper() in target_json:
      ...
      target.append((target_json[file[:5].upper()]) > 3)
...
plt.title("Clasificador de Nivel Educativo: Curva ROC")
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.savefig("education_level_roc.png")

plt.clf()

casos = [u'α+θ+β',u'α',u'θ',u'β']
for i in range(len(cms)):
   plot_confusion_matrix(cms[i], ["Sin estudios superiores","Con estudios superiores"],title=casos[i]+' Matriz de Confusion')
   plt.clf()
