...
def plot_confusion_matrix(cm, classes,
    ...
    plt.tight_layout()
    plt.savefig('sex_' + title.lower().replace(" ","_") + ".png")
...
#         importantes.append(map(lambda x: float(x), clf.support_))
		 ...
		 # is more influential
         #importantes.append(np.std(X_train, 0)*clf.coef_)
...
#   print np.mean(importantes,axis=0)
...
#for filter_i in range(14):	
for filter_i in range(1):
	rootPath = '../../../../informe/IMGs/Open/'

	target_json = json.load(open(rootPath + 'sex.json'))
	...
	for band in ['beta','theta','alpha']:
	  ...
	  for file in os.listdir(rootPath):
	    if file[-4:] == '.csv' and file[:5].upper() in target_json:
	      ...
	      x = list(reader)
#	      x = filterMatrix(x,filter_i)
#	      x = filterMatrix(x,11)
#	      x = filterMatrix(x,4)
	      x = numpy.array(x).astype("float")

	      resting_data[band].append(x)
	      target.append((target_json[file[:5].upper()]) == "Masculino")
	...
	for i in range(len(cms)):
	   plot_confusion_matrix(cms[i], ["Masculino","Femenino"],title=casos[i]+' Matriz de Confusion')
	   plt.clf()

	ress.append((filter_i,res))

for indice, res in ress:
	for v in res:
		print indice
		print v
	print "\n"
