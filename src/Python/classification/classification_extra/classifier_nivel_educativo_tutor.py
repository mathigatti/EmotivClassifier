...
nivel_educativo_tutor = json.load(open(rootPath + 'nivel_educativo_tutor.json'))
...
for band in ['beta','theta','alpha']:
  ...
  for file in os.listdir(rootPath):
    if file[-4:] == '.csv' and file[:5].upper() in nivel_educativo_tutor:
      reader = csv.reader(open(rootPath + file, "rb"), delimiter=",")
      x = list(reader)
      x = numpy.array(x).astype("float")

      resting_data[band].append(x)
      target.append(clases(nivel_educativo_tutor[file[:5].upper()]))
