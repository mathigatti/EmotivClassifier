...
def getTrainDataTarget():
   ...
   data = np.concatenate((unfold_data(resting_data['theta']), unfold_data(resting_data['beta']), unfold_data(resting_data['alpha'])), axis=0)
...
for band in ['beta','theta','alpha']:
  ...
  for file in os.listdir(rootPath):
    if file[-4:] == '.csv' and file[:5].upper() in somnolencia_z:
      ...
      target.append(somnolencia_z[file[:5].upper()] > -1.5)
